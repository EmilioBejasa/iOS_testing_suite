...
- [ ] A test exercises it, under `Tests/<ModuleName>Tests/` (kit-only) or
      `QuoteBoxTests/<ModuleName>Tests.swift` (app-dependent) — see
      CONTRIBUTING.md's decision tree — or via an existing test file, or a
      **Scope note** explains why it's kit-level only
